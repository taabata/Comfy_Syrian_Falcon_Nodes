    ...
    def sample(self, clip,model, add_noise, noise_seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image, text, return_with_leftover_noise,control_net,image,ControlNet_strength, denoise=1.0):
        ...
        for i in range(0,steps-1):
            for idx0,el in enumerate(numloop):
                if el == i+1:
                    if len(prompts)==1:
                        print("no:  "+str(prompts[idx0]))
                        lastprompt = prompts[idx0]
                        newarr = alternatingtokenfunc(prompts[idx0])
                        newprompt = prompts[idx0]
                    else:
                        print("yes:  "+str(prompts[idx0+1]))
                        lastprompt = prompts[idx0+1]
                        newarr = alternatingtokenfunc(prompts[idx0+1])
                        newprompt = prompts[idx0+1]
                    if len(newarr) >=1:
