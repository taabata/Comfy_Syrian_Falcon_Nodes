...
class WordAsImage:
    ...
    def word_as_image(self, text: str, add_depth: str,font: str,rotate: int,add_border:str,size: int,depth_factor:int,border_thickness:int,letter_thickness:int):
        unicode_text = text
        tp = font
        try:
        ...
        canvas = Image.new('L', (text_width+20 + border_thickness+depth_factor, text_height+20 + border_thickness+depth_factor))
        ...
        if add_depth == "enable":
            for i in range(0,depth_factor):
                ...
                x = 255 - int(255/depth_factor)*depth_factor
                draw.text((15+1*i, 12+i*1),unicode_text ,(clr*i+x),font=font,stroke_width = letter_thickness,stroke_fill=(clr*i+x))
        else:
